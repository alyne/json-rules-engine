...
import { EngineFast, Fact, Rule, Operator } from '../../src/index'
...
describe('EngineFast', () => {
  ...
  describe('addRule()', () => {
    describe('rule instance', () => {
      it('adds the rule', () => {
        ...
        // EngineFast converts Rule to RuleFast internally
        expect(engine.rules[0]).to.be.an('object')
        expect(engine.rules[0].event).to.eql(rule.event)
      })
    })
  ...
  describe('removeRule()', () => {
    function setup () {
      ...
      // Return the converted RuleFast instances from the engine
      return [engine.rules[0], engine.rules[1]]
    }
