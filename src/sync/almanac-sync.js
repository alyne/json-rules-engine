...
/**
 * Synchronous fact results lookup
 * Triggers fact computations and saves the results
 * A new almanac is used for every engine run()
 */
export default class AlmanacSync {
  constructor (options = {}) {
    this.factMap = new Map()
    this.factResultsCache = new Map() // { cacheKey: factValue }
  ...
  /**
   * Sets the computed value of a fact (synchronous)
   * @param {FactSync} fact
   ...
   * @param {Object} options - options to initialize the fact with. used when "id" is not a FactSync instance
   */
  ...
  addRuntimeFact (factId, value) {
    ...
    const fact = new FactSync(factId, value)
    return this._addConstantFact(fact)
  }

  /**
   * Returns the value of a fact, based on the given parameters (synchronous)
   * @param  {string} factId - fact identifier
   * @param  {Object} params - parameters to feed into the fact
   * @param  {String} path - object path
   * @return {any} fact computation result
   */
  ...
  /**
   * Interprets value as either a primitive, or if a fact, retrieves the fact value (synchronous)
   */
  getValue (value) {
