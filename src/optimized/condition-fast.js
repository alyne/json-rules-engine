...
export default class ConditionFast {
  constructor (properties) {
    if (!properties) throw new Error('Condition: constructor options required')
    const booleanOperator = ConditionFast.booleanOperator(properties)
    ...
    if (booleanOperator) {
      ...
      if (subConditionsIsArray) {
        this[booleanOperator] = subConditions.map((c) => new ConditionFast(c))
      } else {
        this[booleanOperator] = new ConditionFast(subConditions)
      }
  ...
  toJSON (stringify = true) {
    ...
    const oper = ConditionFast.booleanOperator(this)
    if (oper) {
  ...
  /**
   * Optimized condition evaluation with fast-path for constants
   */
  ...
  /**
   * Optimized boolean evaluation with smart short-circuiting
   */
  ...
  booleanOperator () {
    return ConditionFast.booleanOperator(this)
  }

  isBooleanOperator () {
    return ConditionFast.booleanOperator(this) !== undefined
  }
